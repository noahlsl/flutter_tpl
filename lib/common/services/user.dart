...
class UserService extends GetxService {
  ...
  String token = '';
  final _profile = UserProfileRes(username: '', email: '').obs;

  /// 是否登录
  bool get isLogin => _isLogin.value;

  /// 用户 profile
  UserProfileRes get profile => _profile.value;
  ...
  void onInit() {
    ...
    if (profileOffline.isNotEmpty) {
      _profile(UserProfileRes.fromJson(jsonDecode(profileOffline)));
    }
  }
  ...
  Future<void> getProfile() async {
    ...
    UserProfileRes result = await UserApi.profile();
    _profile(result);
    _isLogin.value = true;
  ...
  /// 设置用户 profile
  Future<void> setProfile(UserProfileRes profile) async {
    if (token.isEmpty) return;
    _isLogin.value = true;
    _profile(profile);
  ...
  Future<void> logout() async {
    ...
    _profile(UserProfileRes(username: '', email: ''));
